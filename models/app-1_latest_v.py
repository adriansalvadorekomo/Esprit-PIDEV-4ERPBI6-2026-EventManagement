...
# ============================================================
    ...
    st.subheader("🚦 Statut de déploiement")
    ...
    d3.metric("Référence naïve (SMAPE)", f"{bench['Naïf']['smape']:.1f}%")
    ...
    elif best_smape < 50 and best_mase < 1.5:
        st.warning(
            f"⚠️ **Déploiement possible avec précautions** — SMAPE {best_smape:.1f}%. "
            f"Prévoir un suivi mensuel et un ré-entraînement automatique."
        )
    else:
        st.error(
    ...
    with st.form("sim_future"):
        ...
        model_choice = sf3.selectbox(
            "Modèle",
            ["Ensemble", "Meilleur benchmark"],
            index=0,
            help="Le benchmark choisit automatiquement le meilleur modèle individuel pour la catégorie sélectionnée."
        )
    ...
    if submit:
        try:
            ...
            if future_cat == cat:
                ...
                future_best_benchmark = best_benchmark_name
            else:
                _, _, future_bench, future_eligible, future_best_benchmark = (
                    train_benchmark_models(df_ts_future)
                )

            if not future_best_benchmark and 'Naïf' in future_bench:
                future_best_benchmark = 'Naïf'

            mc_name = (
                future_best_benchmark
                if model_choice == "Meilleur benchmark" or 'Ensemble' not in future_bench
                else 'Ensemble'
            )
